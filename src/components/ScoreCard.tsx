...
export function ScoreCard({ result, onReset, scoreDiff, initialData, scoreId }: ScoreCardProps) {
  ...
  const handleShare = () => {
    ...
    const shareUrl = scoreId ? `${baseUrl}/report/${scoreId}` : baseUrl;
    
    if (navigator.share && scoreId) {
      navigator.share({
        title: 'My TrustScore Report',
        text: `My TrustScore is ${result.score} (${result.tier})! View my full report here:`,
        url: shareUrl,
      }).catch(console.error);
    } else {
      navigator.clipboard.writeText(shareUrl);
      toast.success("Link copied to clipboard!", {
        description: scoreId ? "Lenders can view your verified report using this link." : "Sign in to generate a permanent verifiable link.",
      });
    }
  };
